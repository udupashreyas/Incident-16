...
img.onload = function(){
    ...
    var terrainTexture = THREE.ImageUtils.loadTexture( 'texture/jotunheimen-texture-white.jpg' );
...
var text0 = new THREE.TextGeometry("NITK Surathkal presents",textParams3);
...
var text7 = new THREE.TextGeometry("PRONITES",textParams3);
var text8 = new THREE.TextGeometry("EVENTS",textParams3);
var text9 = new THREE.TextGeometry("CONTACT",textParams3);
var text10 = new THREE.TextGeometry("HOSPITALITY",textParams3);
...
var textMesh0 = new THREE.Mesh(text0, textMaterialIncident);
...
var textMesh10 = new THREE.Mesh(text10, textMaterial);

textMesh0.position.set(-28,-4,9);
...
textMesh5.position.set(4,-33,7.0);
textMesh6.position.set(-2,16,13.0);
textMesh7.position.set(39,23,8.0);
textMesh8.position.set(-32,-27.5,7.0);
textMesh9.position.set(20,-24.5,8.0);
textMesh10.position.set(-12,-34,7.0);

textMesh0.rotation.x = 3.14/2;
...
var controlsInitialPosition = {x:-6, y:-60, z:25}
var controlsInitialTarget = {x:-6, y:-20, z:0}
...
pointLight.position.set( 6, -33, 6 );
scene.add( pointLight );
...
pointLight.position.set( 23, -29.5, 5 );
scene.add( pointLight );
...
loader.load('models/shield/shield1.obj', 'models/shield/shield1.mtl', function(object) {
    object.position.x = -8;
    ...
    object.scale.y = 0.4;
    object.scale.z = 0.4;
    object.name = "Hospitality"; 
    parent.add( object );
});

var pointLight = new THREE.PointLight( 0xffffff, 1.5, 10 );
pointLight.position.set( -6, -36, 6 );
scene.add( pointLight );
...
function loadedFully(){
    ...
    document.getElementById('myInstructionsModal').style.visibility = "visible";
    Custombox.open({
...
document.onkeydown = function(evt){
    ...
    if(evt.keyCode == 27){
        camera.position.x = controlsInitialPosition.x;
        camera.position.y = controlsInitialPosition.y;
        camera.position.z = controlsInitialPosition.z;
        controls.target.x = controlsInitialTarget.x;
        controls.target.y = controlsInitialTarget.y;
        controls.target.z = controlsInitialTarget.z;
    }
...
function onDocumentMouseDown(event) {
    ...
    if(intersects.length>0) {
        ...
        function tweenBackFunc(){
            var cam_pos_curr = { x : camera.position.x, y : camera.position.y, z : camera.position.z};
            var tweenback = new TWEEN.Tween(cam_pos_curr).to(controlsInitialPosition, 2000);
                    // controls.object.position.set(0, -30, 30);

            tweenback.easing(TWEEN.Easing.Circular.InOut);
            tweenback.onUpdate(function() {
             camera.position.x = cam_pos_curr.x;
             camera.position.y = cam_pos_curr.y;
             camera.position.z = cam_pos_curr.z;
